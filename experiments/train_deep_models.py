...
parser.add_argument('-batch_size_c',  type=int, default=128, help='batch_size') # *** 16 32 64 128
...
for mp_grid in model_collections.mps: # MODEL CONFIGS
	for kmodel_id,model_id in enumerate(model_ids):
		...
		s_train_dataset_da.set_scalers_from(s_train_dataset_da).calcule_precomputed()
		r_train_dataset_da.set_scalers_from(s_train_dataset_da).calcule_precomputed()
		s_train_dataset.set_scalers_from(s_train_dataset_da).calcule_precomputed()
		r_train_dataset.set_scalers_from(s_train_dataset_da).calcule_precomputed()
		r_val_dataset.set_scalers_from(s_train_dataset_da).calcule_precomputed()
		r_test_dataset.set_scalers_from(s_train_dataset_da).calcule_precomputed()
