...
###################################################################################################################################################
	...
	def calcule_ddays_scaler_bdict(self):
		...
		if self.is_parallel:
			...
			values = [diff_vector(self.lcset[lcobj_name].get_custom_x_serial(['days'], max_day=self.max_day)[:,0])[...,None] for lcobj_name in self.get_lcobj_names()]
...
###################################################################################################################################################
	...
	def get_item(self, _lcobj,
		...
		if self.is_parallel:
			d_days = lcobj.get_custom_x_serial(['d_days'], sorted_time_indexs, max_day)
		else:
			d_days = diff_vector(days[...,0])[...,None]
		d_days = self.ddays_normalize(d_days, onehot)
