...
def plot_mse(root_folder,
	...
	metrics_dict = utils.get_day_metrics_from_models(root_folder, model_names, 'mse', fext, error_scale)
	...
	for kmn,model_name in enumerate(model_names):
		...
		ax.plot(days, xe.median, '-', label=f'{utils.formating_model_name(model_name)}', c=colors[idx])
		ax.fill_between(days, xe.p5, xe.p95, alpha=0.25, fc=colors[idx])
		idx += 1

	title = 'reconstruction mse vs days'
...
def plot_f1score_mse(root_folder,
	...
	title = 'f1score vs reconstruction mse'
	title += f'\nsurvey: {survey} - bands: {band_names}'
	ax.set_title(title)
...
def plot_baccu_f1score(root_folder,
	...
	for kax,metric_name in enumerate(metric_names):
		ax = axs[kax]
		metrics_dict = utils.get_day_metrics_from_models(root_folder, model_names, metric_name, fext, error_scale)
		days, survey, band_names, class_names = utils.get_info_from_models(root_folder, model_names, fext)
		index = 0
		for kmn,model_name in enumerate(model_names):
			mn_dict = strings.get_dict_from_string(model_name)
	...
	for kax,metric_name in enumerate(['*precision*', '*recall*']):
		...
		for kmn,model_name in enumerate(model_names):
			...
			if not int(mn_dict['teD']) in [0,4]:
				continue
			xe = metrics_dict[model_name]
			ax.plot(days, xe.median, '--' if int(mn_dict['teD'])==0 else '-', label=f'{utils.formating_model_name(model_name)}', c=colors[idx])
			ax.fill_between(days, xe.p5, xe.p95, alpha=0.25, fc=colors[idx])
			idx += 1
