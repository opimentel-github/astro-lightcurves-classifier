...
def save_attn_scores_animation(train_handler, data_loader, save_rootdir,
	...
	**kwargs):
	results = []
	for experiment_id in range(1): # fixme
		...
		for day in days[::-1]: # along days
			...
			for k,lcobj_name in enumerate(lcobj_names):
				...
				for kb,b in enumerate(dataset.band_names):
					...
					if kb==0:
						ax.axvline(day, linestyle='--', c='k', label=f'day threshold')
					...
					attn_scores_min_max = seq_utils.seq_min_max_norm(attn_scores, dummy_p_onehot) # (b,qt,1)
					attn_scores_min_max = tensor_to_numpy(attn_scores_min_max)
				...
				ax.grid(alpha=0.5)
				if xlims[lcobj_name] is None:
					xlims[lcobj_name] = ax.get_xlim()
